...
def get_posts_by_user(
    ...
    return db_user.get_posts_by_user_id(db, id, current_user_id=current_user.id)
...
def get_friends(id: int, db: Session = Depends(get_db)):
    ...
    friendships = db.query(DbFriendship).filter((DbFriendship.user_id == id)).all()

    all_friendships = []
    is_accepted = False
    for friendship in friendships:
        if friendship.user_id == id:
            friendship_list = get_friendship_by_user(
                db, friendship.user_id, friendship.friend_id
            )
